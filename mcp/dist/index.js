    ...
    switch (name) {
        case 'search_shortages':
            return get('/api/v1/shortages', {
                ...
                atc: args.atc,
                firma: args.firma,
                perPage: Math.min(100, args.limit ?? 20),
            });
        case 'get_shortage':
            return get(`/api/v1/shortages/${encodeURIComponent(args.gtin)}`);
        case 'find_alternatives': {
            // alternatives endpoint takes ?gtin=
            const result = await get('/api/alternatives', { gtin: args.gtin });
            return result;
        }
        case 'check_atc_group': {
            ...
            const data = await get('/api/v1/shortages', {
                atc,
                perPage: Math.min(200, args.limit ?? 50),
            });
        ...
        case 'get_company_status': {
            ...
            const data = await get('/api/v1/shortages', { firma, perPage: 200 });
            const shortages = data.data;
            const statusBreakdown = shortages.reduce((acc, s) => {
        ...
        case 'get_shortage_timeline': {
            const weeks = Math.min(52, args.weeks ?? 12);
            const data = await get('/api/v1/timeline', { weeks });
...
// ── Server setup ──────────────────────────────────────────────────────────────
const server = new Server({ name: 'engpassradar', version: '0.1.0' }, { capabilities: { tools: {} } });
