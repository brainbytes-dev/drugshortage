    ...
    <footer className="border-t mt-auto">
      <div className="max-w-7xl mx-auto px-4 py-5 text-xs text-muted-foreground flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center justify-center flex-wrap gap-x-3 gap-y-1 order-2 sm:order-3 shrink-0">
          <Link href="/impressum" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">{t('impressum')}</Link>
          <span>·</span>
          <Link href="/datenschutz" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">{t('datenschutz')}</Link>
          <span>·</span>
          <Link href="/nutzungsbedingungen" target="_blank" rel="noopener noreferrer" className="underline hover:text-foreground">{t('agb')}</Link>
          <span>·</span>
          <a
        ...
        <p className="text-center order-3 sm:hidden">© {year} {t('copyright')}</p>
        <p className="hidden sm:block order-2 shrink-0">© {year} {t('copyright')}</p>
